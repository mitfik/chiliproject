...
class QueryTest < ActiveSupport::TestCase
  ...
  def test_custom_fields_for_all_projects_should_be_available_in_global_queries
    query = QueryHelper.new(:project => nil, :name => '_')
  ...
  def test_query_should_allow_shared_versions_for_a_project_query
    ...
    query = QueryHelper.new(:project => Project.find(1), :name => '_')
  ...
  def test_user_custom_field_filtered_on_me
    ...
    query = QueryHelper.new(:name => '_', :project => project)
  ...
  def test_filter_watched_issues
    ...
    query = QueryHelper.new(:name => '_', :filters => { 'watcher_id' => {:operator => '=', :values => ['me']}})
    result = find_issues_with_query(query)
  ...
  def test_grouped_with_valid_column
    q = QueryHelper.new(:group_by => 'status')
    assert q.grouped?
  ...
  def test_grouped_with_invalid_column
    q = QueryHelper.new(:group_by => 'foo')
    assert !q.grouped?
  ...
  def test_invalid_query_should_raise_query_statement_invalid_error
    q = QueryHelper.new
    assert_raise QueryHelper::StatementInvalid do
      q.issues(:conditions => "foo = 1")
    end
  end

  def test_issue_count_by_association_group
    q = QueryHelper.new(:name => '_', :group_by => 'assigned_to')
  ...
  def test_issue_count_by_list_custom_field_group
    q = QueryHelper.new(:name => '_', :group_by => 'cf_1')
    count_by_group = q.issue_count_by_group
    assert_kind_of Hash, count_by_group
  ...
  def test_issue_count_by_date_custom_field_group
    q = QueryHelper.new(:name => '_', :group_by => 'cf_8')
    count_by_group = q.issue_count_by_group
    assert_kind_of Hash, count_by_group
